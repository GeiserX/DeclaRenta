...
### FX from foreign-currency stock sales = full proceeds, deferred to conversion (issue #230, Model D)
- **What it does**: a foreign-currency stock SALE now pushes its FCY proceeds into the FX FIFO as an **acquisition** lot (`extractStockProceedsFxEvents` in `fx-fifo.ts`, wired in `report.ts` inside the `skipFx` block as the THIRD event source alongside CASH conversions `extractFxEvents` and dividend/interest inflows `extractCashFxEvents`). Before this, `fifo.ts` and `fx-fifo.ts` were fully decoupled: a foreign-stock sale created no FX lot, so the buy↔sale FX drift that `disposalEur` deliberately strips out of the stock gain (sale-date rate, V2422-20) was simply lost — a later FCY→EUR conversion had nothing to consume.
- **Load-bearing decision — FULL PROCEEDS, not the gain**: the lot quantity is the full net `proceedsFcy` (already net of commission/taxes per `fifo.ts`), NOT `gainLossFcy`. The issue #230 text (and an initial design pass) proposed sizing the lot to the gain — that systematically **UNDER-declares**. The currency received selling a $1200 stock is $1200 of real fungible dollars, not the $200 profit. **Reconciliation identity that proves it**: total economic profit (EUR) = stock-line gain (sale-date EUR) + FX-line gain. Worked example: fund $1200@0.90 (€1080) → buy $1000 stock → sell $1200 → convert $1400@1.05 (€1470); stock line €200, FX line €190, sum **€390 = €1470 − €1080**. The full-proceeds lot reconciles; the gain-only lot does not.
- **Gains AND losses both create the lot**: the stock's P&L sign is irrelevant to the currency received — a loss-making sale still hands you the dollars. There is **NO forced FX disposal on a loss** (rejecting the issue's "Opinion 1") and **NO silent "destruction"** (rejecting the issue's preferred "Opinion 2"). Both opinions answer a malformed question; the FCY is simply received and held. This is a pure ACQUISITION producer — it emits nothing negative.
- **Deferred to conversion (Art. 14.2.e LIRPF)**: the new lot is an acquisition only — it produces **no disposal**. The FX gain/loss is recognized ONLY when the currency is later converted to EUR (the existing conversion-disposal path in `processEvents` consumes these lots). A user who sells FCY stock but never converts sees no FX change — correct per Art. 14.2.e ("se imputará en el momento del cobro o del pago"; DGT V2422-20 / V1613-25 / V0463-21). This is why the `parser-to-casilla` sell-without-convert test stays green.
- **No buy-side handling; why phantom lots are benign**: buys neither create nor consume FX lots, so this does NOT re-introduce the buy-side consumption that PR #171 removed (PR #175 was rejected for it), nor worsen the missing-prior-year-lots phantom problem (see "FX Phantom Gains from Missing Prior-Year Lots"). Because `processEvents` consumes FIFO oldest-first, real funding lots are consumed before sell-proceeds lots; a user can only convert what they actually hold, so over-accumulated sell-proceeds lots sit unconsumed at the tail and are never taxed. Net result matches the **V2324-10 symmetric criterion** (currency FX captured across the full holding period).
- **Doctrinal note + deferred follow-up**: the genuinely-unsettled axis is the BUY side — DGT V2324-10 (older, "symmetric": buying a foreign asset realizes FX on the spent currency) vs V0282-22 / V0152-26 (newer, "defer": no FX realized on buying, only on conversion to EUR). This implementation gives the V2324-10 result **without** buy-side consumption; a V0282-22 "defer-on-buy" variant (which needs buy-side lot consumption) is a documented FUTURE follow-up, not implemented now.
- **Legal-citation correction**: divisa is **Art. 33.1** (transmisión − adquisición), timing **Art. 14.2.e**, FIFO-for-currency rationale **DGT V2324-10**; NOT Art. 37.1.l (which governs "incorporaciones que no derivan de transmisión"). The stale "Art. 37.1.l" comment in `report.ts`'s FX block was corrected alongside this work (see the #225 citation correction above).
- **Scope/filter**: only `assetCategory` STK/FUND/BOND, `currency ≠ EUR`, ecb-resolvable, `proceedsFcy > 0`. This excludes crypto permutas (whose `proceedsFcy` is in the received coin and is meaningless as FX — handled by the crypto valuation path), options/FOP/FSFOP, and the CASH conversions already handled by `extractFxEvents`. Gated by monodivisa (`skipFx`), like the rest of the FX engine.
