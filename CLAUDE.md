...
- **Known limitation — partial-sell-at-a-loss residual (documented, not a bug)**: a partial sell at a LOSS leaves a small bounded residual against the raw-economic total. Tested case: carry-basis FX **€385** vs raw-economic **€405** (−€20). This is the genuine V2422-20 "two-element" gap — the stock loss consumes principal whose FX drift is attributed to NEITHER the stock line NOR the FX line; no DGT consulta resolves it, and forcing the figure to the raw-economic number would be its own distortion. Accepted and documented, like the monodivisa `totalSavingsBase` limitation.
